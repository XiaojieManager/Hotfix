...
//  from limboy - http://limboy.me/tech/2018/03/04/ios-lightweight-hotfix.html

#import "Felix.h"

@implementation Felix
+ (Felix *)shareInstance {
    static Felix *shareInstance = nil;
...
+ (void)_fixWithMethod:(BOOL)isClassMethod aspectionOptions:(AspectOptions)option instanceName:(NSString *)instanceName selectorName:(NSString *)selectorName fixImpl:(JSValue *)fixImpl{
    ...
    [klass aspect_hookSelector:sel withOptions:option usingBlock:^(id<AspectInfo> aspecInfo){
        [fixImpl callWithArguments:@[aspecInfo.instance, aspecInfo.originalInvocation, aspecInfo.arguments]];
    } error:nil];
}
